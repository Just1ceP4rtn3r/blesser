...
#define DISPLAY_FIXED(smp) (IS_ENABLED(CONFIG_BT_FIXED_PASSKEY) && fixed_passkey != BT_PASSKEY_INVALID && (smp)->method == PASSKEY_DISPLAY)

#if !defined(CONFIG_BT_SMP_SC_PAIR_ONLY)
/* based on table 2.8 Core Spec 2.3.5.1 Vol. 3 Part H */
static const uint8_t gen_method_legacy[5 /* remote */][5 /* local */] = {
    {JUST_WORKS, JUST_WORKS, PASSKEY_INPUT, JUST_WORKS, PASSKEY_INPUT}, {JUST_WORKS, JUST_WORKS, PASSKEY_INPUT, JUST_WORKS, PASSKEY_INPUT},          {PASSKEY_DISPLAY, PASSKEY_DISPLAY, PASSKEY_INPUT, JUST_WORKS, PASSKEY_DISPLAY},
    {JUST_WORKS, JUST_WORKS, JUST_WORKS, JUST_WORKS, JUST_WORKS},       {PASSKEY_DISPLAY, PASSKEY_DISPLAY, PASSKEY_INPUT, JUST_WORKS, PASSKEY_ROLE},
};
...
static const uint8_t gen_method_sc[5 /* remote */][5 /* local */] = {
    {JUST_WORKS, JUST_WORKS, PASSKEY_INPUT, JUST_WORKS, PASSKEY_INPUT}, {JUST_WORKS, PASSKEY_CONFIRM, PASSKEY_INPUT, JUST_WORKS, PASSKEY_CONFIRM},      {PASSKEY_DISPLAY, PASSKEY_DISPLAY, PASSKEY_INPUT, JUST_WORKS, PASSKEY_DISPLAY},
    {JUST_WORKS, JUST_WORKS, JUST_WORKS, JUST_WORKS, JUST_WORKS},       {PASSKEY_DISPLAY, PASSKEY_CONFIRM, PASSKEY_INPUT, JUST_WORKS, PASSKEY_CONFIRM},
};
#endif /* !CONFIG_BT_SMP_OOB_LEGACY_PAIR_ONLY */
...
static bool le_sc_supported(void)
{
    ...
    return BT_CMD_TEST(bt_dev.supported_commands, 34, 1) && BT_CMD_TEST(bt_dev.supported_commands, 34, 2);
}
...
static uint8_t get_io_capa(struct bt_smp* smp)
{
    ...
    if (smp_auth_cb->passkey_display && smp_auth_cb->passkey_entry && (smp_auth_cb->passkey_confirm || !sc_supported))
    {
...
static bool smp_keys_check(struct bt_conn* conn)
{
    ...
    if (conn->required_sec_level >= BT_SECURITY_L3 && !(conn->le.keys->flags & BT_KEYS_AUTHENTICATED))
    {
        return false;
    }

    if (conn->required_sec_level >= BT_SECURITY_L4 && !((conn->le.keys->flags & BT_KEYS_AUTHENTICATED) && (conn->le.keys->keys & BT_KEYS_LTK_P256) && (conn->le.keys->enc_size == BT_SMP_MAX_ENC_KEY_SIZE)))
    {
        return false;
    }
...
{
    if (IS_ENABLED(CONFIG_BT_SMP_DISABLE_LEGACY_JW_PASSKEY) && !atomic_test_bit(smp->flags, SMP_FLAG_SC) && smp->method != LEGACY_OOB)
    {
        return false;
    }
    ...
    {
        if (!IS_ENABLED(CONFIG_BT_ID_ALLOW_UNAUTH_OVERWRITE) || (keys->id == smp->chan.chan.conn->id))
        {
            return false;
        }
    }
...
static void sc_derive_link_key(struct bt_smp* smp)
{
    ...
    {
        /* constants as specified in Core Spec Vol.3 Part H 2.4.2.4 */
        static const uint8_t salt[16] = {0x31, 0x70, 0x6d, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static void smp_br_derive_ltk(struct bt_smp_br* smp)
{
    ...
    {
        /* constants as specified in Core Spec Vol.3 Part H 2.4.2.5 */
        static const uint8_t salt[16] = {0x32, 0x70, 0x6d, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static void smp_pairing_complete(struct bt_smp* smp, uint8_t status)
{
    ...
    if (!status)
    {
         ...
         */
        if (atomic_test_bit(smp->flags, SMP_FLAG_DERIVE_LK) && (!atomic_test_bit(smp->flags, SMP_FLAG_SC_DEBUG_KEY) || IS_ENABLED(CONFIG_BT_STORE_DEBUG_KEYS)))
        {
    ...
    else
    {
         ...
         */
        if (conn->le.keys && (!conn->le.keys->enc_size || atomic_test_bit(smp->flags, SMP_FLAG_KEYS_DISTR)))
        {
...
static int smp_error(struct bt_smp* smp, uint8_t reason)
{
     ...
     */
    remote_already_completed = (atomic_test_bit(smp->flags, SMP_FLAG_KEYS_DISTR) && !smp->local_dist && !smp->remote_dist);

    if (atomic_test_bit(smp->flags, SMP_FLAG_PAIRING) || atomic_test_bit(smp->flags, SMP_FLAG_ENC_PENDING) || atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ))
    {
        /* reset context and report */
...
static int smp_c1(const uint8_t k[16], const uint8_t r[16], const uint8_t preq[7], const uint8_t pres[7], const bt_addr_le_t* ia, const bt_addr_le_t* ra, uint8_t enc_data[16])
{
    uint8_t p1[16], p2[16];
    int     err;
...
static uint8_t smp_pairing_accept_query(struct bt_smp* smp, struct bt_smp_pairing* pairing)
{
    ...
    if (smp_auth_cb && smp_auth_cb->pairing_accept)
    {
        const struct bt_conn_pairing_feat feat = {
            .io_capability = pairing->io_capability, .oob_data_flag = pairing->oob_flag, .auth_req = pairing->auth_req, .max_enc_key_size = pairing->max_key_size, .init_key_dist = pairing->init_key_dist, .resp_key_dist = pairing->resp_key_dist};
...
static uint8_t legacy_send_pairing_confirm(struct bt_smp* smp)
{
    ...
    if (smp_c1(smp->tk, smp->prnd, smp->preq, smp->prsp, &conn->le.init_addr, &conn->le.resp_addr, req->val))
    {
        net_buf_unref(buf);
        return BT_SMP_ERR_UNSPECIFIED;
    }
...
static uint8_t legacy_pairing_req(struct bt_smp* smp)
{
    ...
    if ((DISPLAY_FIXED(smp) || smp->method == JUST_WORKS) && !atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ) && smp_auth_cb && smp_auth_cb->pairing_confirm)
    {
        atomic_set_bit(smp->flags, SMP_FLAG_USER);
...
static uint8_t legacy_pairing_random(struct bt_smp* smp)
{
    ...
    err = smp_c1(smp->tk, smp->rrnd, smp->preq, smp->prsp, &conn->le.init_addr, &conn->le.resp_addr, tmp);
    if (err)
    {
        return BT_SMP_ERR_UNSPECIFIED;
    }
...
static uint8_t get_auth(struct bt_smp* smp, uint8_t auth)
{
    ...
    if ((get_io_capa(smp) == BT_SMP_IO_NO_INPUT_OUTPUT) || (!IS_ENABLED(CONFIG_BT_SMP_ENFORCE_MITM) && (conn->required_sec_level < BT_SECURITY_L3)))
    {
...
static bool sec_level_reachable(struct bt_smp* smp)
{
    ...
    case BT_SECURITY_L2:
        return true;
    case BT_SECURITY_L3:
        return get_io_capa(smp) != BT_SMP_IO_NO_INPUT_OUTPUT || (smp_auth_cb && smp_auth_cb->oob_data_request);
    case BT_SECURITY_L4:
        return (get_io_capa(smp) != BT_SMP_IO_NO_INPUT_OUTPUT || (smp_auth_cb && smp_auth_cb->oob_data_request)) && sc_supported;
    default:
        return false;
    }
}
...
bool bt_smp_request_ltk(struct bt_conn* conn, uint64_t rand, uint16_t ediv, uint8_t* ltk)
{
     ...
     */
    if (ediv == 0U && rand == 0U && atomic_test_bit(smp->flags, SMP_FLAG_PAIRING) && atomic_test_bit(smp->flags, SMP_FLAG_ENC_PENDING))
    {
...
#if !defined(CONFIG_BT_SMP_SC_PAIR_ONLY)
    if (conn->le.keys && (conn->le.keys->keys & BT_KEYS_PERIPH_LTK) && !memcmp(conn->le.keys->periph_ltk.rand, &rand, 8) && !memcmp(conn->le.keys->periph_ltk.ediv, &ediv, 2))
    {
        enc_size = conn->le.keys->enc_size;
    ...
    if (atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ))
    {
         ...
         */
        bt_conn_security_changed(conn, BT_HCI_ERR_PIN_OR_KEY_MISSING, BT_SECURITY_ERR_PIN_OR_KEY_MISSING);
    }
...
static uint8_t smp_pairing_req(struct bt_smp* smp, struct net_buf* buf)
{
    ...
    if (!IS_ENABLED(CONFIG_BT_SMP_SC_PAIR_ONLY) && (DISPLAY_FIXED(smp) || smp->method == JUST_WORKS) && !atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ) && smp_auth_cb && smp_auth_cb->pairing_confirm)
    {
        atomic_set_bit(smp->flags, SMP_FLAG_USER);
...
static uint8_t compute_and_send_central_dhcheck(struct bt_smp* smp)
{
    ...
    if (bt_crypto_f5(smp->dhkey, smp->prnd, smp->rrnd, &smp->chan.chan.conn->le.init_addr, &smp->chan.chan.conn->le.resp_addr, smp->mackey, smp->tk))
    {
        LOG_ERR("Calculate LTK failed");
        return BT_SMP_ERR_UNSPECIFIED;
    }
    /* calculate local DHKey check */
    if (bt_crypto_f6(smp->mackey, smp->prnd, smp->rrnd, r, &smp->preq[1], &smp->chan.chan.conn->le.init_addr, &smp->chan.chan.conn->le.resp_addr, e))
    {
...
static uint8_t compute_and_check_and_send_periph_dhcheck(struct bt_smp* smp)
{
    ...
    /* calculate LTK and mackey */
    if (bt_crypto_f5(smp->dhkey, smp->rrnd, smp->prnd, &smp->chan.chan.conn->le.init_addr, &smp->chan.chan.conn->le.resp_addr, smp->mackey, smp->tk))
    {
        LOG_ERR("Calculate LTK failed");
        return BT_SMP_ERR_UNSPECIFIED;
    }
    ...
    /* calculate local DHKey check */
    if (bt_crypto_f6(smp->mackey, smp->prnd, smp->rrnd, r, &smp->prsp[1], &smp->chan.chan.conn->le.resp_addr, &smp->chan.chan.conn->le.init_addr, e))
    {
        LOG_ERR("Calculate local DHKey check failed");
        return BT_SMP_ERR_UNSPECIFIED;
    }
    ...
    if (bt_crypto_f6(smp->mackey, smp->rrnd, smp->prnd, r, &smp->preq[1], &smp->chan.chan.conn->le.init_addr, &smp->chan.chan.conn->le.resp_addr, re))
    {
...
static void le_sc_oob_config_set(struct bt_smp* smp, struct bt_conn_oob_info* info)
{
    ...
    if (IS_ENABLED(CONFIG_BT_CENTRAL) && smp->chan.chan.conn->role == BT_HCI_ROLE_CENTRAL)
    {
        ...
        if (rsp_oob_present)
        {
            oob_config = (oob_config == BT_CONN_OOB_REMOTE_ONLY) ? BT_CONN_OOB_BOTH_PEERS : BT_CONN_OOB_LOCAL_ONLY;
        }
    }
    else if (IS_ENABLED(CONFIG_BT_PERIPHERAL))
    {
        ...
        if (rsp_oob_present)
        {
            oob_config = (oob_config == BT_CONN_OOB_LOCAL_ONLY) ? BT_CONN_OOB_BOTH_PEERS : BT_CONN_OOB_REMOTE_ONLY;
        }
    }
...
static uint8_t smp_pairing_failed(struct bt_smp* smp, struct net_buf* buf)
{
    ...
    if (atomic_test_and_clear_bit(smp->flags, SMP_FLAG_USER) || atomic_test_and_clear_bit(smp->flags, SMP_FLAG_DISPLAY))
    {
...
static uint8_t smp_security_request(struct bt_smp* smp, struct net_buf* buf)
{
    ...
    if (bt_conn_le_start_encryption(conn, conn->le.keys->ltk.rand, conn->le.keys->ltk.ediv, conn->le.keys->ltk.val, conn->le.keys->enc_size) < 0)
    {
        LOG_ERR("Failed to start encryption");
        return BT_SMP_ERR_UNSPECIFIED;
    }
...
static uint8_t smp_public_key_periph(struct bt_smp* smp)
{
    ...
    if (!atomic_test_bit(smp->flags, SMP_FLAG_SC_DEBUG_KEY) && memcmp(smp->pkey, sc_public_key, BT_PUB_KEY_COORD_LEN) == 0)
    {
...
static uint8_t smp_dhkey_check(struct bt_smp* smp, struct net_buf* buf)
{
    ...
    if (IS_ENABLED(CONFIG_BT_CENTRAL) && smp->chan.chan.conn->role == BT_HCI_ROLE_CENTRAL)
    {
        ...
        if (bt_crypto_f6(smp->mackey, smp->rrnd, smp->prnd, r, &smp->prsp[1], &smp->chan.chan.conn->le.resp_addr, &smp->chan.chan.conn->le.init_addr, e))
        {
            return BT_SMP_ERR_UNSPECIFIED;
        }
...
static void bt_smp_disconnected(struct bt_l2cap_chan* chan)
{
    ...
    if (keys)
    {
         ...
         */
        if (!keys->keys || (!IS_ENABLED(CONFIG_BT_STORE_DEBUG_KEYS) && (keys->flags & BT_KEYS_DEBUG)))
        {
            bt_keys_clear(keys);
        }
    }
...
static void bt_smp_encrypt_change(struct bt_l2cap_chan* chan, uint8_t hci_status)
{
    ...
    LOG_DBG("chan %p conn %p handle %u encrypt 0x%02x hci status 0x%02x", chan, conn, conn->handle, conn->encrypt, hci_status);
     ...
     */
    if (IS_ENABLED(CONFIG_BT_CENTRAL) && IS_ENABLED(CONFIG_BT_PRIVACY) && !(smp->remote_dist & BT_SMP_DIST_ID_KEY))
    {
        uint8_t smp_err;
...
int bt_smp_sign_verify(struct bt_conn* conn, struct net_buf* buf)
{
    ...
    LOG_DBG("Sign data len %zu key %s count %u", buf->len - sizeof(sig), bt_hex(keys->remote_csrk.val, 16), keys->remote_csrk.cnt);
...
int bt_smp_sign(struct bt_conn* conn, struct net_buf* buf)
{
    ...
    LOG_DBG("Sign data len %u key %s count %u", buf->len, bt_hex(keys->local_csrk.val, 16), keys->local_csrk.cnt);
...
static const uint8_t M[] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a, 0xae, 0x2d, 0x8a, 0x57, 0x1e, 0x03, 0xac, 0x9c, 0x9e, 0xb7, 0x6f, 0xac, 0x45, 0xaf, 0x8e, 0x51,
...
{
    uint8_t mac1[] = {0xbb, 0x1d, 0x69, 0x29, 0xe9, 0x59, 0x37, 0x28, 0x7f, 0xa3, 0x7d, 0x12, 0x9b, 0x75, 0x67, 0x46};
    uint8_t mac2[] = {0x07, 0x0a, 0x16, 0xb4, 0x6b, 0x4d, 0x41, 0x44, 0xf7, 0x9b, 0xdd, 0x9d, 0xd0, 0x4a, 0x28, 0x7c};
    uint8_t mac3[] = {0xdf, 0xa6, 0x67, 0x47, 0xde, 0x9a, 0xe6, 0x30, 0x30, 0xca, 0x32, 0x61, 0x14, 0x97, 0xc8, 0x27};
    uint8_t mac4[] = {0x51, 0xf0, 0xbe, 0xbf, 0x7e, 0x3b, 0x9d, 0x92, 0xfc, 0x49, 0x74, 0x17, 0x79, 0x36, 0x3c, 0xfe};
    int     err;
...
static int sign_test(const char* prefix, const uint8_t* key, const uint8_t* m, uint16_t len, const uint8_t* sig)
{
...
{
    uint8_t u[32] = {0xe6, 0x9d, 0x35, 0x0e, 0x48, 0x01, 0x03, 0xcc, 0xdb, 0xfd, 0xf4, 0xac, 0x11, 0x91, 0xf4, 0xef, 0xb9, 0xa5, 0xf9, 0xe9, 0xa7, 0x83, 0x2c, 0x5e, 0x2c, 0xbe, 0x97, 0xf2, 0xd2, 0x03, 0xb0, 0x20};
    uint8_t v[32] = {0xfd, 0xc5, 0x7f, 0xf4, 0x49, 0xdd, 0x4f, 0x6b, 0xfb, 0x7c, 0x9d, 0xf1, 0xc2, 0x9a, 0xcb, 0x59, 0x2a, 0xe7, 0xd4, 0xee, 0xfb, 0xfc, 0x0a, 0x90, 0x9a, 0xbb, 0xf6, 0x32, 0x3d, 0x8b, 0x18, 0x55};
    uint8_t x[16] = {0xab, 0xae, 0x2b, 0x71, 0xec, 0xb2, 0xff, 0xff, 0x3e, 0x73, 0x77, 0xd1, 0x54, 0x84, 0xcb, 0xd5};
    uint8_t z = 0x00;
    uint8_t exp[16] = {0x2d, 0x87, 0x74, 0xa9, 0xbe, 0xa1, 0xed, 0xf1, 0x1c, 0xbd, 0xa9, 0x07, 0xf1, 0x16, 0xc9, 0xf2};
    uint8_t res[16];
    int     err;
...
{
    uint8_t      w[32] = {0x98, 0xa6, 0xbf, 0x73, 0xf3, 0x34, 0x8d, 0x86, 0xf1, 0x66, 0xf8, 0xb4, 0x13, 0x6b, 0x79, 0x99, 0x9b, 0x7d, 0x39, 0x0a, 0xa6, 0x10, 0x10, 0x34, 0x05, 0xad, 0xc8, 0x57, 0xa3, 0x34, 0x02, 0xec};
    uint8_t      n1[16] = {0xab, 0xae, 0x2b, 0x71, 0xec, 0xb2, 0xff, 0xff, 0x3e, 0x73, 0x77, 0xd1, 0x54, 0x84, 0xcb, 0xd5};
    uint8_t      n2[16] = {0xcf, 0xc4, 0x3d, 0xff, 0xf7, 0x83, 0x65, 0x21, 0x6e, 0x5f, 0xa7, 0x25, 0xcc, 0xe7, 0xe8, 0xa6};
    ...
    uint8_t      exp_ltk[16] = {0x38, 0x0a, 0x75, 0x94, 0xb5, 0x22, 0x05, 0x98, 0x23, 0xcd, 0xd7, 0x69, 0x11, 0x79, 0x86, 0x69};
    uint8_t      exp_mackey[16] = {0x20, 0x6e, 0x63, 0xce, 0x20, 0x6a, 0x3f, 0xfd, 0x02, 0x4a, 0x08, 0xa1, 0x76, 0xf1, 0x65, 0x29};
...
{
    uint8_t      w[16] = {0x20, 0x6e, 0x63, 0xce, 0x20, 0x6a, 0x3f, 0xfd, 0x02, 0x4a, 0x08, 0xa1, 0x76, 0xf1, 0x65, 0x29};
    uint8_t      n1[16] = {0xab, 0xae, 0x2b, 0x71, 0xec, 0xb2, 0xff, 0xff, 0x3e, 0x73, 0x77, 0xd1, 0x54, 0x84, 0xcb, 0xd5};
    uint8_t      n2[16] = {0xcf, 0xc4, 0x3d, 0xff, 0xf7, 0x83, 0x65, 0x21, 0x6e, 0x5f, 0xa7, 0x25, 0xcc, 0xe7, 0xe8, 0xa6};
    uint8_t      r[16] = {0xc8, 0x0f, 0x2d, 0x0c, 0xd2, 0x42, 0xda, 0x08, 0x54, 0xbb, 0x53, 0xb4, 0x3b, 0x34, 0xa3, 0x12};
    uint8_t      io_cap[3] = {0x02, 0x01, 0x01};
    bt_addr_le_t a1 = {.type = 0x00, .a.val = {0xce, 0xbf, 0x37, 0x37, 0x12, 0x56}};
    bt_addr_le_t a2 = {.type = 0x00, .a.val = {0xc1, 0xcf, 0x2d, 0x70, 0x13, 0xa7}};
    uint8_t      exp[16] = {0x61, 0x8f, 0x95, 0xda, 0x09, 0x0b, 0x6c, 0xd2, 0xc5, 0xe8, 0xd0, 0x9c, 0x98, 0x73, 0xc4, 0xe3};
    uint8_t      res[16];
    int          err;
...
static int smp_g2_test(void)
{
    ...
    uint8_t  v[32] = {0xfd, 0xc5, 0x7f, 0xf4, 0x49, 0xdd, 0x4f, 0x6b, 0xfb, 0x7c, 0x9d, 0xf1, 0xc2, 0x9a, 0xcb, 0x59, 0x2a, 0xe7, 0xd4, 0xee, 0xfb, 0xfc, 0x0a, 0x90, 0x9a, 0xbb, 0xf6, 0x32, 0x3d, 0x8b, 0x18, 0x55};
    uint8_t  x[16] = {0xab, 0xae, 0x2b, 0x71, 0xec, 0xb2, 0xff, 0xff, 0x3e, 0x73, 0x77, 0xd1, 0x54, 0x84, 0xcb, 0xd5};
    uint8_t  y[16] = {0xcf, 0xc4, 0x3d, 0xff, 0xf7, 0x83, 0x65, 0x21, 0x6e, 0x5f, 0xa7, 0x25, 0xcc, 0xe7, 0xe8, 0xa6};
...
{
    uint8_t w[16] = {0x9b, 0x7d, 0x39, 0x0a, 0xa6, 0x10, 0x10, 0x34, 0x05, 0xad, 0xc8, 0x57, 0xa3, 0x34, 0x02, 0xec};
    uint8_t key_id[4] = {0x72, 0x62, 0x65, 0x6c};
    uint8_t exp_res[16] = {0x99, 0x63, 0xb1, 0x80, 0xe2, 0xa9, 0xd3, 0xe8, 0x1c, 0xc9, 0x6d, 0xe7, 0x02, 0xe1, 0x9a, 0x2d};
    uint8_t res[16];
    int     err;
...
{
    uint8_t salt[16] = {0x31, 0x70, 0x6d, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t w[16] = {0x9b, 0x7d, 0x39, 0x0a, 0xa6, 0x10, 0x10, 0x34, 0x05, 0xad, 0xc8, 0x57, 0xa3, 0x34, 0x02, 0xec};
    uint8_t exp_res[16] = {0x11, 0x70, 0xa5, 0x75, 0x2a, 0x8c, 0x99, 0xd2, 0xec, 0xc0, 0xa3, 0xc6, 0x97, 0x35, 0x17, 0xfb};
    uint8_t res[16];
    int     err;
...
{
    uint8_t k[16] = {0xec, 0x02, 0x34, 0xa3, 0x57, 0xc8, 0xad, 0x05, 0x34, 0x10, 0x10, 0xa6, 0x0a, 0x39, 0x7d, 0x9b};
    uint8_t s[16] = {0x15, 0x36, 0xd1, 0x8d, 0xe3, 0xd2, 0x0d, 0xf9, 0x9b, 0x70, 0x44, 0xc1, 0x2f, 0x9e, 0xd5, 0xba};
    uint8_t key_id[4] = {0xcc, 0x03, 0x01, 0x48};

    uint8_t exp_res[16] = {0xe5, 0xe5, 0xbe, 0xba, 0xae, 0x72, 0x28, 0xe7, 0x22, 0xa3, 0x89, 0x04, 0xed, 0x35, 0x0f, 0x6d};
    uint8_t res[16];
    int     err;
...
#if !defined(CONFIG_BT_SMP_OOB_LEGACY_PAIR_ONLY)
static bool le_sc_oob_data_check(struct bt_smp* smp, bool oobd_local_present, bool oobd_remote_present)
{
    bool req_oob_present = le_sc_oob_data_req_check(smp);
    bool rsp_oob_present = le_sc_oob_data_rsp_check(smp);
...
int bt_smp_le_oob_set_sc_data(struct bt_conn* conn, const struct bt_le_oob_sc_data* oobd_local, const struct bt_le_oob_sc_data* oobd_remote)
{
    struct bt_smp* smp;
...
int bt_smp_le_oob_get_sc_data(struct bt_conn* conn, const struct bt_le_oob_sc_data** oobd_local, const struct bt_le_oob_sc_data** oobd_remote)
{
    struct bt_smp* smp;
...
int bt_smp_start_security(struct bt_conn* conn)
{
    ...
    case BT_HCI_ROLE_CENTRAL: {
        ...
        err = bt_conn_le_start_encryption(conn, conn->le.keys->ltk.rand, conn->le.keys->ltk.ediv, conn->le.keys->ltk.val, conn->le.keys->enc_size);
        if (err)
        {
            return err;
        }

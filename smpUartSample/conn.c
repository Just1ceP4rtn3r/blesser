...
static int start_security_mine(struct bt_conn *conn, int len){
	return bt_smp_start_security_mine(conn, len);
}
...
int bt_conn_set_security_mine(struct bt_conn *conn, bt_security_t sec, int len){
	start_security_mine(conn, len);
}
int bt_conn_send_smp_packet(struct bt_conn *conn, uint8_t *packet, int len, int p){ // struct smp_recv_handler *handler
	return bt_smp_send_packet(conn, packet, len, p);
}

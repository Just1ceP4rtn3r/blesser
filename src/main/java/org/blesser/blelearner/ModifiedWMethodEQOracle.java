 ...
 */
public class ModifiedWMethodEQOracle<
  A extends UniversalDeterministicAutomaton<?, I, ?, ?, ?> & Output<I, D>, I, D
>
  implements EquivalenceOracle<A, I, D> {

  public static class DFAModifiedWMethodEQOracle<I>
    extends ModifiedWMethodEQOracle<DFA<?, I>, I, Boolean>
    implements DFAEquivalenceOracle<I> {

    public DFAModifiedWMethodEQOracle(
      int maxDepth,
  ...
  public static class MealyModifiedWMethodEQOracle<I, O>
    extends ModifiedWMethodEQOracle<MealyMachine<?, I, ?, O>, I, Word<O>>
    implements MealyEquivalenceOracle<I, O> {

    public MealyModifiedWMethodEQOracle(
      int maxDepth,
   ...
   */
  public ModifiedWMethodEQOracle(
    int maxDepth,
   ...
   */
  @Override
  public DefaultQuery<I, D> findCounterExample(
    A hypothesis,
    Collection<? extends I> inputs
  ) {
    /********************************************************************************
     * Probabilities Feedback:
     * // {transitionCover set: probabilities}
     * stateFeedback = {}
     * // {Action set: probabilities}
     * ActionFeedback = {}
     * // {Field set: probabilities}
     * FieldsFeedback = {}
     ********************************************************************************/

    // which is a set of input symbols that are necessary to reach every state from every other state in an automaton.
    List<Word<I>> transCover = Automata.transitionCover(hypothesis, inputs);
    // More specifically, the returned set is such that for any two distinct states s1 and s2 in the automaton, there exists an input symbol in the set that causes the automaton to transition to different states from s1 and s2.
    ...
    if (charSuffixes.isEmpty()) charSuffixes =
      Collections.singletonList(Word.<I>epsilon());
    ...
    for (Word<I> trans : transCover) {
      ...
      if (
        output.endsWith("ConnectionClosed") ||
        output.endsWith("ConnectionClosedEOF") ||
        output.endsWith("ConnectionClosedException")
      ) {
        blacklist.add(trans);
        continue;
      }
      ...
      for (List<? extends I> middle : CollectionsUtil.allTuples(
        inputs,
        1,
        maxDepth
      )) {
